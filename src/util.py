...
def prep_collection_table(df, original_df, metrics):
    ...
    for collection in metrics["collections"]:
        ...
        subset_topics = set([tc for row in subset_df["Text Topics"] for tc in row])
        ...
        table["Generated By"].append(subset_model_gen[0] if subset_model_gen else "-")
        table["Mean Input Words"].append(subset_mean_inp_len)
        table["Mean Target Words"].append(subset_mean_tar_len)
